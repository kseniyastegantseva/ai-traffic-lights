...
## 2026-08-25 — Исследовательские графики эффективности

- В dashboard добавлен отдельный раздел «Исследовательские графики».
- Добавлены графики среднего времени ожидания, пропускной способности,
  улучшения относительно `fixed`, средней длины очереди и распределения
  ожидания по random seed.
- Добавлен график динамики очереди во времени с усреднением по random seed;
  секундные значения очереди сохраняются в подробных результатах симуляции.
- На графике среднего ожидания отображается 95% доверительный интервал.
- Графики строятся по `outputs/experiment_suite_results.json`, который создаётся
  командой `traffic-sim compare --config configs/experiment_suite.yaml`.
- Проверки в текущей среде ограничены: Linux Python отсутствует, а Docker daemon
  недоступен; добавлен unit-тест контрактов исследовательских графиков.

## 2026-08-25 — Интеграция детектора и dashboard

- Обе части перенесены в корневой репозиторий; вложенные каталоги и git-репозитории удалены.
- YOLO-детектор перенесён в `src/traffic_light/vision_detector.py`, веса — в `models/`.
- Dashboard получил сценарий «загрузка фотографии → автоматическое распознавание → симуляция».
- Ручной ввод очередей удалён; значения направлений берутся из результата детекции.
- Добавлены зависимости `ultralytics` и `opencv-python-headless`.
- Проверена компиляция Python-модулей через `py -3 -m compileall -q src app tests`.
- Полный pytest не запущен: в окружении отсутствует пакет `pytest`.
...
## 2026-08-15 — Инструкция запуска на новом устройстве

- README расширен разделом `Запуск на новом устройстве`.
- Добавлены шаги установки Git, Docker Desktop и Python 3.12, клонирования репозитория,
  запуска dashboard через Docker и запуска без Docker.
- Отдельно описан вариант `docker-compose.ci.yml` для Windows Docker Desktop без WSL
  integration и необходимость пересборки dashboard после изменений.
- Добавлены команды быстрой проверки `ruff check .` и `pytest -q`.
- Код проекта не менялся; запуск тестов не требовался для документационной правки.

## 2026-08-15 — Проверка push и безопасная остановка Docker

- Перед остановкой Docker выполнен `git fetch origin main` и проверено, что локальная
  ветка `main` совпадает с `origin/main`.
- На момент проверки последним синхронизированным коммитом был
  `d64ef2f Document setup on a new device`.
- Был обнаружен запущенный контейнер `traffic_light-dashboard-1` на порту 8501.
- Код проекта не менялся; тесты не запускались, так как задача была эксплуатационной.
