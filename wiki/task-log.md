...
## 2026-08-28 — Обновление модели распознавания на сайте

- В проект добавлен клон репозитория `gregkorneev/computer-vision` в каталоге
  `computer-vision/`.
- Dashboard теперь использует дообученную на VisDrone модель
  `models/visdrone_vehicles.pt` вместо COCO-модели `yolo11m.pt`.
- В `vision_detector.py` обновлены идентификаторы классов на `0: car`, `1: motorcycle`,
  `2: bus`, `3: truck`; сохранены JSON-контракт dashboard и многомасштабная обработка.
- Добавлен контрактный тест выбора новой модели.
- Проверки и Docker-запуск выполняются после завершения интеграции.

## 2026-08-25 — Исследовательские графики эффективности

- В dashboard добавлен блок «Исследовательские графики» внизу второго экрана
  после симуляции; отдельная кнопка или навигационный раздел не используются.
...
- Графики строятся по `outputs/experiment_suite_results.json`, который создаётся
  автоматически при первом открытии второго экрана dashboard.
- Если актуального результата нет, dashboard сам запускает полный набор
  сценариев и random seed из `configs/experiment_suite.yaml`.
...
## 2026-08-25 — Размещение исследовательских графиков

- Первый экран загрузки фотографии возвращён к исходному виду без кнопки или
  подсказки про исследовательские графики.
- Графики перенесены непосредственно под блок симуляции на втором экране.
- Добавлен тест, запрещающий появление кнопки навигации «Исследовательские графики».

## 2026-08-25 — Интерактивные графики по сценарию фотографии

- Графики сделаны крупными и интерактивными: включены панель Plotly,
  масштабирование, hover и responsive-режим.
- По умолчанию dashboard выбирает исследовательский сценарий по нагрузке,
  определённой на загруженной фотографии.
- Переключение сценария выполняется компактными кнопками рядом с графиками;
  выбранный набор графиков заменяет предыдущий.
- Добавлено пояснение назначения стратегий `fixed`, `actuated` и `ai`.

## 2026-08-25 — Упрощение сравнения графиков

- Из графиков пользовательского экрана исключена стратегия `actuated`;
  оставлены только обычный `fixed` и интеллектуальный `ai`.
- Убраны кнопки масштабирования, прокрутка-zoom и drag-zoom; сохранены
  наведение курсора и адаптивное отображение.
